...
model.output.to.plot <- function(ABC.out.mat, par.vec.length, iter, time.steps, vars.to.plot, init.date.data="2020-01-03",all=NULL,scenario.selection,intervention_date=NULL,sd.redux=NULL) {
...
model.output.to.table <- function(ABC.out.mat, par.vec.length, iter, time.steps, init.date.data="2020-01-03") {
  ...
  rownames(quick.facts) <- c("Peak Hospitalizations", "Deaths by August 1, 2020", "Detected Illnesses by August 1,2020", "Total Illnesses by August 1, 2020", "Proportion of Cases Detected (%)", "CFR Based on Observed Illnesses (%)", "CFR Based on Total Illnesses (%)", "R0 - before social distancing", "% Reduction in Social Contacts (March 15 - )")
...
plot.model.data.all <- function(traj.CI, data.in, init.date.data, date.offset.4plot, time.steps.4plot, vars.to.plot) {
  ...
  traj.CI.area <- dcast(traj.CI.area, "date+state.name+CI~type")
