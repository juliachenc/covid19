...
####################################################################################
## GET POPULATION FREQUENCY FUNCTION FOR H, Q, AND D POPULATIONS
...
get.Pr.q <- function(model, time, logit.SEIR.est, X.mat, freq.q.IN,  psi.mat){
  ...
  time <- time
  model <- model
  logit.SEIR.est.m.t <- as.numeric(logit.SEIR.est[time, model]) #DEFINE
  logit.SEIR.est.m.t.VEC <- rep( logit.SEIR.est.m.t,  times=nrow(X.mat))
  psi.m.vec <- psi.mat[,model] #DEFINE
  #freq.IN.q <- freq.IN[[2]]
  n.profiles <- nrow(X.mat)
  freq.q.IN.mat <- matrix(rep(freq.q.IN,each=n.profiles),nrow=n.profiles)

  # GET PROBABILITY VECTOR
  pre.expit <- logit.SEIR.est.m.t.VEC + (X.mat - freq.q.IN.mat) %*% psi.m.vec
...
# DEFINE PSI (in a table)
# DEFINE LOGIT.SEIR.EST (from epidemic model)
