...
supporting_functions <- path(code.dir, "Oct_data/supporting_fns_Alpha_t_NEW.R")
...
prior.r1 <- c("unif",0.078, 0.08) # 0.079
...
ABC_Marjoram<-ABC_mcmc(method="Marjoram",model=model.R,prior=prior.par,
                       ...
                       tolerance_quantile=0.1,verbose=TRUE,progress=TRUE, n_rec=100) #, dist_weights=dist_weights )
...
par.vec.length <- 100
iter <- 10
time.steps <- 300 #as.numeric(Sys.Date() - as.Date("2020-03-01")) + start_time 
...
time.steps.4.plot = as.numeric(Sys.Date() - as.Date("2020-03-01")) + start_time  #160 
data.in <- data.in.tmp
vars.to.plot <- vars.plus.R
## Plot all
