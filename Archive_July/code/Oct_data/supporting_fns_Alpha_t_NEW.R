...
#
      ...
      # ####################################
      # ##### Beta_y readin 10.13.20 ######

      mu_t_readin_path <- path(data.dir, "mu_t_readin.csv")
      mu_t_readin = as.data.frame(read.csv(mu_t_readin_path, sep=",",stringsAsFactors = FALSE))

      Beta_t <- as.Date(mu_t_readin$Beta_t)
      Beta_t <- as.numeric(Beta_t - as.Date("2020-03-01")) + start_time
      Beta_t[1] <- Beta_t[1]-start_time

      mu_y_chr <- mu_t_readin$Beta_y
      assign("mu.0",1)
      assign("mu.1", R0_redux1)  #assign("mu.1", .2)
      assign("mu.2", R0_redux2)  #assign("mu.2",.45)
      assign("mu.3",2*(mu.2 - mu.1))
      ...
      for (i in 1:length(Beta_t)){
        Beta_y[i] = Br.function(R0.in=R0_y[i], r.in=r1, Alpha.in=Alpha1)
      }

      Beta_y<- c(
        Br.function(R0.in<-R0_y[1], r.in<-r1, Alpha.in<-Alpha1) , #1
        Br.function(R0.in<-R0_y[2], r.in<-r1, Alpha.in<-Alpha1), #2
        Br.function(R0.in<-R0_y[3], r.in<-r1, Alpha.in<-Alpha1), #3
        Br.function(R0.in<-R0_y[4], r.in<-r1, Alpha.in<-Alpha1), #4
        Br.function(R0.in<-R0_y[5], r.in<-r1, Alpha.in<-Alpha1), #  (Br*R0_redux1), #5
        Br.function(R0.in<-R0_y[6], r.in<-r2, Alpha.in<-Alpha2), #  (Br*R0_redux1), #6
        Br.function(R0.in<-R0_y[7], r.in<-r2, Alpha.in<-Alpha2), #  (Br*R0_redux2), #7
        Br.function(R0.in<-R0_y[8], r.in<-r2, Alpha.in<-Alpha2), #  (Br*R0_redux2*.9), #8
        ...
        Br.function(R0.in<-R0_y[12], r.in<-r2, Alpha.in<-Alpha2)#   (Br*R0_redux2*.9)) #9
      )
