...
summary.table <- function(ABC.out.mat, par.vec.length, iter, time.steps, init.date.data="2020-03-01") {
  ...
  rownames(posterior.vars) <- c("r, fraction obs. illnesses", "R0 Initial", "R0 fraction reduction", "Pr(Hospital|Illness)", "Pr(ICU|Hospital)", "Pr(Death|ICU)")
  ...
  for (i in 1:nrow(ABC.out.mat)) {
    ...
    if (scenario==0){
      ...
      Beta_t<- c(
        ...
        (start_time+todays_date_numeric  +50),#  -5),
