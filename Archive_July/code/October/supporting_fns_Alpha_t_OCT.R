...
summary.table.DATES <- function(traj.CI, ABC.out.mat) {
  #summary.table <- function(traj.CI, ABC.out.mat, par.vec.length, iter, time.steps, init.date.data="2020-03-01") {

  library(data.table)

  ## GET TODAY'S DATE
  CFR.1<- traj.CI %>% filter(date==as.Date("2020-04-20"),state.name=="CFRobs") %>% select(-c(date,N,mean,up_50,low_50)) %>% mutate_if(is.numeric, round, digits=4)
  IFR.1<- traj.CI %>% filter(date==as.Date("2020-04-20"),state.name=="CFRactual") %>% select(-c(date,N,mean,up_50,low_50)) %>% mutate_if(is.numeric, round, digits=4)
  FR.1 <- rbind(CFR.1,IFR.1)
  rownames(FR.1) <- c("CFR Apr.20", "IFR Apr.20")
  FR.1 <- select(FR.1, -c(1))

  CFR.2<- traj.CI %>% filter(date==as.Date("2020-07-20"),state.name=="CFRobs") %>% select(-c(date,N,mean,up_50,low_50)) %>% mutate_if(is.numeric, round, digits=4)
  IFR.2<- traj.CI %>% filter(date==as.Date("2020-07-20"),state.name=="CFRactual") %>% select(-c(date,N,mean,up_50,low_50)) %>% mutate_if(is.numeric, round, digits=4)
  FR.2 <- rbind(CFR.2,IFR.2)
  rownames(FR.2) <- c("CFR Jul.20", "IFR Jul.20")
  FR.2 <- select(FR.2, -c(1))
  FR <- rbind(FR.1,FR.2)
  rownames(FR) <- c(rownames(FR.1),rownames(FR.2))
  ...
  all.posterior.vars <- join(posterior.vars,FR,type="full") #rbind(CFR.today, posterior.vars)
  rownames(all.posterior.vars) <- c(rownames(posterior.vars),rownames(FR))
  colnames(all.posterior.vars) <- c("Lower 95% CI", "Median", "Upper 95% CI")
...
#summary.table.DATES(traj.CI=traj.0, ABC.out.mat=ABC.out.mat)
...
########################################################################################
## SPECIFYING beta(t) FUNCTIONS TO ESTIMATE AND SIMULATE
########################################################################################
  ...
  for (i in 1:nrow(ABC.out.mat)) {
    ...
    # Beta_y_obs <- Br*(1-R0_redux)
     ...
     if (scenario==4){
      ...
      #Alpha_t_CAL <- Alpha_t - start_time + as.Date("2020-03-01")
      Alpha_t_TEST2 <- c(
        as.Date("2020-03-01")-start_time ,
        as.Date("2020-04-30"),
        as.Date("2020-05-01"),
        as.Date("2020-05-17"),
        as.Date("2022-06-21")
      )
      Alpha_t_TEST2_numeric <- as.numeric(Alpha_t_TEST2 - as.Date("2020-03-01") + start_time)
      #Alpha_t_TEST2_numeric_CAL <- Alpha_t_TEST2_numeric - start_time + as.Date("2020-03-01")
      Alpha_t <- Alpha_t_TEST2_numeric


      # Alpha_t_TEST_3 <- c(
      #   as.Date("2020-03-01")-start_time ,
      #   as.Date("2020-06-01") - 1,
      #   as.Date("2020-06-01"),
      #   as.Date("2020-07-01"),
      #   as.Date("2022-06-21")
      # )
      # Alpha_t_TEST_3_numeric <- as.numeric(Alpha_t_TEST_3 - as.Date("2020-03-01") + start_time)
      # Alpha_t <- Alpha_t_TEST_3_numeric
      ...
      Beta_t_TEST_ORIG_numeric_SIMMODEL <- as.numeric(Beta_t_TEST_ORIG - as.Date("2020-03-01")) + start_time
      #Beta_t_TEST_ORIG_numeric_SIMMODEL_CAL <- Beta_t_TEST_ORIG_numeric_SIMMODEL - start_time + as.Date("2020-03-01")
      Beta_t <- Beta_t_TEST_ORIG_numeric_SIMMODEL


      # Beta_t_TEST<- c(
      #   0 - start_time,                              #1
      #   (start_time - start_time),                #2
      #   (start_time+1 - start_time),                #3
      #   (start_time+intervention_date_numeric - start_time),   #2020-03-12    #4
      #   (start_time+intervention_date_numeric+15 - start_time),#2020-03-27    #5
      #   (start_time+intervention_date_numeric+46 - start_time),#2020-04-27    #6
      #   (start_time+intervention_date_numeric+114 - start_time),#2020-07-04   #7
      #   (start_time+todays_date_numeric - start_time),   #CURRENT DATE         #8
      #   (start_time+todays_date_numeric+400- start_time))#DATE FAR IN FUTURE   #9
      ...
      Beta_y<- c(
        Br.function(R0.in<-R0, r.in<-r1, Alpha.in<-Alpha1) , #1
        Br.function(R0.in<-R0, r.in<-r1, Alpha.in<-Alpha1), #2
        Br.function(R0.in<-R0, r.in<-r1, Alpha.in<-Alpha1), #3
        Br.function(R0.in<-R0, r.in<-r1, Alpha.in<-Alpha1), #4
        Br.function(R0.in<-R0*R0_redux1, r.in<-r1, Alpha.in<-Alpha1), #  (Br*R0_redux1), #5
        Br.function(R0.in<-R0*R0_redux1, r.in<-r1, Alpha.in<-Alpha2), #  (Br*R0_redux1), #6
        Br.function(R0.in<-R0*R0_redux2, r.in<-r2, Alpha.in<-Alpha2), #  (Br*R0_redux2), #7
        Br.function(R0.in<-R0*R0_redux2*.9, r.in<-r2, Alpha.in<-Alpha2), #  (Br*R0_redux2*.9), #8
        Br.function(R0.in<-R0*R0_redux2*.9, r.in<-r2, Alpha.in<-Alpha2) #   (Br*R0_redux2*.9)) #9
      )
      ...
      r_t_DATES <- c(
        ...
        as.Date("2020-04-15"),
        as.Date("2020-08-15"),
        as.Date("2020-08-15")+400
      )
      r_t_NUMERIC <- as.numeric(r_t_DATES - as.Date("2020-03-01") + start_time)
      #Beta_t_TEST_ORIG_CAL <- Beta_t_TEST_ORIG_numeric - start_time + as.Date("2020-03-01")
      r_t <- round(r_t_NUMERIC)
      r_y <- c(
        r1,
        r1,
        r2,
    ...
    #x <- seihqdr_generator(Beta_t=Beta_t, Beta_y=Beta_y, S_ini=1e7,E_ini=10,r=r,Delta = Delta, Alpha=Alpha, Kappa=Kappa, p_QV=p_V)
    #x <- seihqdr_generator(Beta_t=Beta_t, Beta_y=Beta_y, S_ini=1e7,E_ini=10,r=r,Delta = Delta, Kappa=Kappa, p_QV=p_V)
    x <- seihqdr_generator(Alpha_t=Alpha_t, Alpha_y=Alpha_y, Kappa_t=Kappa_t, Kappa_y=Kappa_y, Delta_t=Delta_t, Delta_y=Delta_y, r_t=r_t, r_y=r_y, Beta_t=Beta_t, Beta_y=Beta_y, S_ini=1e7, E_ini=10, p_QV=p_V)

    ## SIMULATE
    TEST<-as.data.frame(plyr::rdply(iter, x$run(0:(time.steps+start_time)),.id="iter"))
...
model.1sim.stats.no.R <- function(par){
  ...
  #   Sys.Date() + 400
  # )
  # Beta_t_TEST2_numeric <- as.numeric(Beta_t_TEST2 - as.Date("2020-03-01") + start_time)
  # Beta_t_TEST2_numeric_CAL <- Beta_t_TEST2_numeric - start_time + as.Date("2020-03-01")
  ...
  Beta_t_TEST_ORIG_numeric <- as.numeric(Beta_t_TEST_ORIG - as.Date("2020-03-01") + start_time)
  #Beta_t_TEST_ORIG_CAL <- Beta_t_TEST_ORIG_numeric - start_time + as.Date("2020-03-01")
  Beta_t <- Beta_t_TEST_ORIG_numeric
  ...
  Beta_y<- c(
    Br.function(R0.in<-R0, r.in<-r, Alpha.in<-Alpha1) , #1
    Br.function(R0.in<-R0, r.in<-r, Alpha.in<-Alpha1), #2
    Br.function(R0.in<-R0, r.in<-r, Alpha.in<-Alpha1), #3
    Br.function(R0.in<-R0, r.in<-r, Alpha.in<-Alpha1), #4
    Br.function(R0.in<-R0*R0_redux1, r.in<-r, Alpha.in<-Alpha1), #  (Br*R0_redux1), #5
    Br.function(R0.in<-R0*R0_redux1, r.in<-r, Alpha.in<-Alpha2), #  (Br*R0_redux1), #6
    Br.function(R0.in<-R0*R0_redux2, r.in<-r, Alpha.in<-Alpha2), #  (Br*R0_redux2), #7
    Br.function(R0.in<-R0*R0_redux2*.9, r.in<-r, Alpha.in<-Alpha2), #  (Br*R0_redux2*.9), #8
    Br.function(R0.in<-R0*R0_redux2*.9, r.in<-r, Alpha.in<-Alpha2) #   (Br*R0_redux2*.9)) #9
  )
  ...
  ### GENERATE SIMULATION
  x <- seihqdr_generator(Alpha_t=Alpha_t, Alpha_y=Alpha_y, Kappa_t=Kappa_t, Kappa_y=Kappa_y, Delta_t=Delta_t, Delta_y=Delta_y, Beta_t=Beta_t, Beta_y=Beta_y, S_ini=1e7, E_ini=10, p_QV=p_V)
  st <- start_time
  one_sim <- as.data.frame(x$run(1:(st+no_obs))[(st+1):(st+no_obs),])
...
model.1sim.stats.yes.R <- function(par){
  ...
  Alpha_t_TEST2_numeric <- as.numeric(Alpha_t_TEST2 - as.Date("2020-03-01") + start_time)
  #Alpha_t_TEST2_numeric_CAL <- Alpha_t_TEST2_numeric - start_time + as.Date("2020-03-01")
  Alpha_t <- round(Alpha_t_TEST2_numeric)
  #
  # Alpha_t_TEST_3 <- c(
  #   as.Date("2020-03-01")-start_time ,
  #   as.Date("2020-06-01") - 1,
  ...
  # )
  # Alpha_t_TEST_3_numeric <- as.numeric(Alpha_t_TEST_3 - as.Date("2020-03-01") + start_time)
  # Alpha_t <- round(Alpha_t_TEST_3_numeric)
  ...
  Beta_t_TEST_ORIG_numeric <- as.numeric(Beta_t_TEST_ORIG - as.Date("2020-03-01") + start_time)
  #Beta_t_TEST_ORIG_CAL <- Beta_t_TEST_ORIG_numeric - start_time + as.Date("2020-03-01")
  Beta_t <- round(Beta_t_TEST_ORIG_numeric)
