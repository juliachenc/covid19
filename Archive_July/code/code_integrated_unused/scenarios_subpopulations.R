...
subpop.scenarios <- function(ABC.out.mat,iter,time.steps,scenario=NULL,Alpha.filter=NULL,Kappa.filter=NULL,Delta.filter=NULL) {
  ...
  for (i in 1:nrow(ABC.out.mat)) {
    ...
    #Alpha.input <- Alpha2 #Alpha.filter
    #Kappa.input <- Kappa2 #Kappa.filter
    #Delta.input <- Delta2 #Delta.filter
    Alpha.input <- Alpha.filter
    Kappa.input <- Kappa.filter
    Delta.input <- Delta.filter
    ...
    ## SCENARIO 1: R0 DO NOTHING + SHIELD SPECIFIC POPULATIONS
      ...
      Alpha_y<-c(
        Alpha1, # 0,
        Alpha1, # Alpha_decrease_date,
        Alpha.input, # Alpha_stabilize_date,
        Alpha.input # Alpha_stabilize_date + 400
      )
      Kappa_t <- Alpha_t
      Kappa_y<-c(
        Kappa1, # 0,
        Kappa1, # Alpha_decrease_date,
        Kappa.input, # Alpha_stabilize_date,
        Kappa.input # Alpha_stabilize_date + 400
      )
      Delta_t <- Alpha_t
      Delta_y<-c(
        Delta1, # 0,
        Delta1, # Alpha_decrease_date,
        Delta.input, # Alpha_stabilize_date,
        Delta.input # Alpha_stabilize_date + 400
      )
    ...
    ## COMPILE
    x <- seihqdr_generator(Alpha_t=Alpha_t, Alpha_y=Alpha_y, Kappa_t=Kappa_t, Kappa_y=Kappa_y, Delta_t=Delta_t, Delta_y=Delta_y, Beta_t=Beta_t, Beta_y=Beta_y, S_ini=8e6, E_ini=10, r=r, p_QV=p_V)
...
model.output.to.plot.subpop.scenarios <- function(ABC.out.mat, par.vec.length, iter, time.steps, vars.to.plot, init.date.data="2020-03-01",scenario, Alpha.filter=NULL,Kappa.filter=NULL,Delta.filter=NULL){

  library(data.table)

  ## MODEL OUTPUT TO PLOT
  TEST.out <- subpop.scenarios(ABC.out.mat[1:par.vec.length,],iter=iter,time.steps=time.steps, scenario=scenario, Alpha.filter,Kappa.filter,Delta.filter)
...
# Alpha.filter <- Alpha.filter.prior.mean.50
# Kappa.filter <- Kappa.filter.prior.mean.50
# Delta.filter <- Delta.filter.prior.mean.50
