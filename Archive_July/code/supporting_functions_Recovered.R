...
summary.table <- function(traj.CI, ABC.out.mat) {
  ...
  posterior.vars <- rbind(r.posterior.CI, R0.posterior.CI, R0redux1.posterior.CI, R0redux2.posterior.CI, Alpha.posterior.CI,Kappa.posterior.CI,Delta.posterior.CI) %>% mutate_if(is.numeric, round, digits=4)
  rownames(posterior.vars) <- c("r, fraction obs. illnesses", "R0 Initial", "R0 fraction reduction Mar12","R0 fraction reduction July5", "Pr(Hospital|Illness)", "Pr(ICU|Hospital)", "Pr(Death|ICU)")
...
#     (start_time+todays_date_numeric + 75),
#     (start_time+500))
#
#   Beta_y<- c(
#     Br, #1
