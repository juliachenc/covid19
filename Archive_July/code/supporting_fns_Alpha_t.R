...
correlated.param.SIM <- function(ABC.out.mat,iter,time.steps,startObservedData,scenario=NULL,intervention_date=NULL,sd.redux=NULL) {
  ...
  todays_date_numeric = as.numeric(as.Date("2020-08-30")-as.Date("2020-03-01")) #as.numeric(Sys.Date() - as.Date("2020-03-01"))
  ...
  for (i in 1:nrow(ABC.out.mat)) {
    ...
    if (scenario==0){
      ## NO INTERVENTIONS: PROJECTION USING MODEL ESTIMATED PARAMETERS
      Beta_t<- c(
        0,                              #1
        (start_time),                #2
        (start_time+1),                #3
      ...
      ## SCENARIO 2: SHEILD SPECIFIC POPULATIONS
      Alpha_decrease_date <- as.numeric( as.Date("2020-03-19") - as.Date("2020-03-01"))
      Alpha_stabilize_date <- as.numeric( as.Date("2020-03-20") - as.Date("2020-03-01"))
      ...
      Alpha_y<-c(
        ...
        Alpha.filter, # Alpha_stabilize_date,
        Alpha.filter # Alpha_stabilize_date + 400
      )
      Kappa_t <- Alpha_t
      Kappa_y<-c(
        Kappa1, # 0,
        Kappa1, # Alpha_decrease_date,
        Kappa.filter, # Alpha_stabilize_date,
        Kappa.filter # Alpha_stabilize_date + 400
      )
      Delta_t <- Alpha_t
      Delta_y<-c(
        Delta1, # 0,
        Delta1, # Alpha_decrease_date,
        Delta.filter, # Alpha_stabilize_date,
        Delta.filter # Alpha_stabilize_date + 400
      )
