...
- $I$, Number of infected cases, **detected and observed** with testing
...
- $H$, Patients hospitalized (including those in the ICU)
- $Q$, Patients in the ICU
- $V$, Patients on ventilators
- $D$, Deaths
...
## Why our model is unique: Contributions according to each aim
...
State that R0 ~ Beta ~ contact rate x probability of transmission → R0 ~ contact rate 
→ Drop in R0 is ~ to drop in contact rate
...
### TRY THIS FOR IMAGES
knitr::include_graphics(path(data.dir, "flow_diagram_v2.png"))
...
- Values were taken from the literature from various, summarized [here](https://docs.google.com/spreadsheets/d/1PQttOS0VllpDGZswngMBIX5GMV2cMAxyseD3L-aNw-U/edit#gid=1798107562).
...
### We estimate the following key epidemic and model quantities
...
**In the following we provide information on prior specification and resulting posterior estimation of each parameter. Because our model is stochastic and we are using Bayesian techniques for parameter estimation, each posterior parameter estimate is represented by a distribution of likely values.**
...
* **Probability of health care resource utilization**: Our model provides the “baseline” estimates of health care probabilities due to biological risk factors for COVID alone (age, comorbidities, obesity, smoking). However there are many other factors that are impacting what we are observing on the epidemic --
  ...
  * **Access to care**: Disparities exist in access to health care and which individuals are being treated in health care facilities, which also impacts the numbers showing up in health care related data.
...
* L.A. County has succeeded in mitigating the epidemic curve due to early action and strong adherence to social distancing (model-estimated reduction in contact rate of >70%).
* Our model suggests that social distancing may be relaxed to levels even up to an 100% increase of social encounters beyond that enforced in the first phase of response (Level 1), while still keeping the epidemic under control. However beyond a certain level of increase, a major resurgence will be seen. What exactly does an 100% increase from Level 1 social distancing translate to? Finer-resolution micro-simulation models combined with analysis of near-real-time mobility data may help to answer these questions.
* Our model predicts that higher risk groups -- characterized not only by advanced age but also combinations of existing health conditions -- will make up the overwhelming majority of those afflicted with severe disease. Our model breaks down risk to the various combinations of various risk factors. The results of this analysis show that advance age in combination with multiple comorbidities leads the burden of morbidity and mortality for COVID-19.
* We analyze how these risk factors project onto race/ethnicity groups and project expected illness patterns based on population size and prevalence of these risk factors. Thus far, the observed risk of death by race in L.A. is consistent with what we would expect based on biological risk factors alone. This will need to be further analyzed and interpreted, but as an initial analysis, may suggest that LA may be an outlier compared with other major U.S. cities in terms of the disproportionate burden of morbidity and mortality on more marginalized groups.
