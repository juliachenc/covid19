...
endDatePlotGlobal <- as.Date("2021-05-16")
```
...
prior.R0 = c("normal", prior_readin[1,3], 0.1*prior_readin[1,3])
prior.r = c("normal", prior_readin[1,4], .25*prior_readin[1,4])
prior.Delta = c("normal", prior_readin[1,5], .1*prior_readin[1,5])
prior.Alpha = c("normal", prior_readin[1,6], .1*prior_readin[1,6])
prior.Kappa = c("normal", prior_readin[1,7], .1*prior_readin[1,7])
prior.p_V = c("normal", prior_readin[1,8], .1*prior_readin[1,8])
...
week_par_all[[1]] <- cbind(data.frame(week.id = 1, iter.id = 1:nrow(ABC.par.out_week_1)), ABC.par.out_week_1)
```
...
prior.R0 = c("normal", prior_readin[2,3], 0.001*prior_readin[2,3])
prior.r = c("normal", prior_readin[2,4], .25*prior_readin[2,4])
prior.Delta = c("normal", prior_readin[2,5], 0.001*prior_readin[2,5])
prior.Alpha = c("normal", prior_readin[2,6], 0.001*prior_readin[2,6])
prior.Kappa = c("normal", prior_readin[2,7], 0.001*prior_readin[2,7])
prior.p_V = c("normal", prior_readin[2,8], 0.001*prior_readin[2,8])
...
## LOAD DATA PROCESSING AND MODEL ESTIMATING FUNCTIONS
model_functions_week2<- path(code.paper.dir, "data_model_fun.R")
...
week_par_all[[2]] <- cbind(data.frame(week.id = 2, iter.id = 1:nrow(ABC.par.out_week_2)), ABC.par.out_week_2)
```
...
## SUMMARY STATISTICS COMPUTED ON DATA
week.no = c(2:86) #c(3:52)
#week.no=Vectorize(week.no)

for (i in week.no){
  prior.R0 = c("normal", prior_readin[i,3], 0.001*prior_readin[i,3])
  prior.r = c("normal", prior_readin[i,4], .01*prior_readin[i,4])
  prior.Delta = c("normal", prior_readin[i,5], 0.001*prior_readin[i,5])
  prior.Alpha = c("normal", prior_readin[i,6], 0.001*prior_readin[i,6])
  prior.Kappa = c("normal", prior_readin[i,7], 0.001*prior_readin[i,7])
  prior.p_V = c("normal", prior_readin[i,8], 0.001*prior_readin[i,8])
  ...
  model_functions_week3later<- path(code.paper.dir, "data_model_fun.R")
  source(model_functions_week3later)
    ...
    for (j in 1:nrow(ABC.par.out_week)){
        ABC.par.out_week$V1[j] <- max(ABC.par.out_week$V1[j] , .0075)
        ABC.par.out_week$V2[j] <- max(ABC.par.out_week$V2[j] , .0075)
        ABC.par.out_week$V3[j] <- max(ABC.par.out_week$V3[j] , .0075)
        ABC.par.out_week$V4[j] <- max(ABC.par.out_week$V4[j] , .0075)
        ABC.par.out_week$V5[j] <- max(ABC.par.out_week$V5[j] , .0075)
        ABC.par.out_week$V6[j] <- max(ABC.par.out_week$V6[j] , .0075)
    }
    week_par_mean[i,] = colMeans(ABC.par.out_week)
    week_par_sd[i,] = colStdevs(ABC.par.out_week)
    week_par_all[[i]] <- cbind(data.frame(week.id = i, iter.id = 1:nrow(ABC.par.out_week)), ABC.par.out_week)
}
```
...
week_par_sim <- do.call(rbind, week_par_all)
max_week = max(week_par_sim$week.id)-2

#ABC.out.mat <- ABC_out$param
#par.vec.length <- length(week_par_sim)
iter <- 10
time.steps <- max_week*7+45
...
traj.0 <- model.output.to.plot.SIM(week_par_sim=week_par_sim, iter=iter, time.steps=time.steps, vars.to.plot = vars.to.plot)
```
...
#time.steps.4.plot = as.Date("2021-01-17") - as.Date("2020-03-01") 
time.steps.4.plot = endDatePlotGlobal - as.Date("2020-03-01")
data.in <- latest_data
vars.to.plot <- vars.plus.R

plot.all.variables.curr.date <- plot.model.data.all(traj.CI = traj.0, data.in = data.in, init.date.data = "2020-03-01", date.offset.4plot = 0, time.steps.4plot=time.steps.4.plot, vars.to.plot=vars.plus.R)
