...
##CITY DATA
city_data <- city_data("Boyle Heights")
```


<!--Dataframe for Mean, SD and all-->
```{r}
...
week_par_all <- vector("list", n_rec)
```
...
for (i in week.no){
  ...
  source(model_functions)
    ...
    summarydata <- sum.stats.SIMTEST(la_data[c(1:(7*i+1)),])
    ...
    for (j in 1:nrow(ABC.par.out_week)){
        ABC.par.out_week$V1[j] <- max(ABC.par.out_week$V1[j] , .007)
...
# traj.city <- model.output.to.plot.SIM(week_par_sim=week_par_sim, iter=iter, time.steps=time.steps, vars.to.plot = vars.to.plot)
