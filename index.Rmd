...
### @EMIL: EDITED THIS FUNCTION 5-19-20
...
NH_frac <- function(NH.rm.start.date,no_obs,NH.var.to.modify,input.NH.dates, full.data.in,var.to.modify){
  ...
  # # t0 = assume.first.NH.date
  # # tf = input.NH.dates[1]+1
  # # y0 = 1
  # # yf = NH.var.to.modify[1]
  # # 
  ...
  # # NH.counts[t_int] <- round(y_int)
  # # NH.counts[(tf+1):no_obs] <- NH.var.to.modify[-1]
  # 
...
**The black dots in the figures below depict COVID-19 data with counts from outbreaks in nursing homes removed (for details see Methods).**
...
### @EMIL: THIS WHOLE CHUNK I'VE EDITED
...
#  ggplot( aes(x=stage,y=values,fill=factor(group,levels=c(race.order))))+
  geom_bar(position="stack", stat = 'identity')+
  labs(title = "Model-Projected Relative Proportion (%) of Race/Ethnicity \n by Stage of Disease", x = "Stage", y = "Relative Proportion (%)", legend="Race/Ethnicity") +
...
D.dates.select <- c("2020-04-16", "2020-05-01", "2020-05-11", "2020-05-17")
...
labels.shares.races.D <- c("% of population", paste0("Obs. % of deaths \n",D.dates.select), "Model projected % \n of deaths ")
