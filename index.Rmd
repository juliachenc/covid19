...
We do not provide a model-estimated distribution of illness by death because this is not accounted for in our model; we have assumed randomly-mixed population in which every individual has the risk of infection. This limitation is discussed further in [Methods](https://uscbiostats.github.io/COVID19/method.html).
...
## Death given Illness by Race {.tabset}

### Pr(Death|Illness) by Race/Ethnicity
...
# # RR.DgivenPop.pop <- RR.calc(race_prevalence_gen_pop,"White")
# # RR.DgivenPop.pop
# 
# ### MODEL
# RR.DgivenPop.Model <- RR.calc(race_model_D, "White") 
# RR.DgivenPop.Model
# 
# ### OBS
# race_observed_D_recent <- as.numeric(race_obs_D[,ncol(race_obs_D)]) # CHOOSING THE MOST RECENT DEATH OBSERVATION - SHARES
# names(race_observed_D_recent) <- names(race_model_D)
# RR.DgivenPop.Obs <- RR.calc(race_observed_D_recent, "White")
# RR.DgivenPop.Obs
# 
# ### TABLE
# # RR.DgivenPop.desc <- as.data.frame(cbind(RR.DgivenPop.Model,RR.DgivenPop.Obs))
# # #RR.DgivenPop.desc <- RR.DgivenPop[order(-RR.DgivenPop$RR.DgivenPop.Model), , drop = FALSE]
# # colnames(RR.DgivenPop.desc) <- c("Race to White: Model","Race to White: Data")
# # max.RR <- max(RR.DgivenPop.desc)
# # min.RR <- min(RR.DgivenPop.desc)
# # mat.scale = function(x) (x) / (max.RR)
# # RR.DgivenPop.table <- formattable(RR.DgivenPop.desc,
# #             list(
# #               #`Race to White: Pop` = color_bar('red', fun = mat.scale),
# #               `Race to White: Model` = color_bar('lightblue', fun = mat.scale),
# #               `Race to White: Data` = color_bar('violet', fun = mat.scale)
# #             ))
...
# ### TABLE
# RR.DgivenI.desc <- as.data.frame(cbind(RR.vec.DgivenI.model,RR.vec.DgivenI.obs))
# colnames(RR.DgivenI.desc) <- c("Race to White: Model","Race to White: Data")
# max.RR <- max(RR.DgivenI.desc)
# min.RR <- min(RR.DgivenI.desc)
# mat.scale = function(x) (x) / (max.RR)
# 
# RR.DgivenI.table <- formattable(RR.DgivenI.desc,
#             list(
#               `Race to White: Model` = color_bar('lightpink', fun = mat.scale),
#               `Race to White: Data` = color_bar('salmon', fun = mat.scale)
#             ))

# ######################
# ## BAR CHART (PROBABLY DELETE THIS)
# 
...
#   labs(title = "Relative Risk Death given Illness: \n Races to White", x = NULL, y = "Relative Risk to White", legend="Model vs. Obs")
...
### Model vs. Observed Relative Risk by Race

This table compares the observed vs. model estimated **Relative Risk (RR)** of the probability of death given illness, $Pr(Death|Illness)$ for each race/ethnicity group relative to $Pr(Death|Illness)$ for whites in L.A. County, i.e.

$$
\begin{equation*}
RR_{White}(Death|Illness) = \frac{Pr(Death|Illness,Race_i)}{Pr(Death|Illness,White)} \
\end{equation*}
$$
...
colnames(RR.DgivenI.desc) <- c("RR_White(Death|Illness): Model","RR_White(Death|Illness): Data")
max.RR <- max(RR.DgivenI.desc)
min.RR <- min(RR.DgivenI.desc)
mat.scale = function(x) (x) / (max.RR)

formattable(RR.DgivenI.desc,
            list(
