...
D.dates.select <- c("2020-04-22", "2020-05-01", "2020-05-08", "2020-05-15", "2020-05-22","2020-05-29","2020-06-05","2020-06-12","2020-06-15")
...
#race_observed_D_recent.nominal <- as.numeric(race_observed_D_select[,ncol(race_observed_D_select)]) # CHOOSING THE MOST RECENT DEATH OBSERVATION - NOMINAL
race_obs_I_recent <- race_observed_I %>% select(ncol(race_observed_I)) # CHOOSING THE MOST RECENT DEATH OBSERVATION - NOMINAL
Pr.DgivenI.obs <- as.numeric(t(race_observed_D_recent.nominal / race_obs_I_recent))
...
num.to.remove.I.tot <- NH_total_frac_I * sum(race_obs_I_recent)   
# NH_total_frac_I is the most recent
num.to.remove.I.race <- round(SNF.race.distribution * num.to.remove.I.tot)
race_obs_I_recent.REMOVED <- race_obs_I_recent - num.to.remove.I.race
...
D.per.Pop.obs <- race_observed_D_recent.nominal / Pr.pop #as.vector(t(race_obs_I_recent))/Pr.pop
names(D.per.Pop.obs) <- names(race_model_D)
...
race_obs_I_recent.REMOVED.frac <- apply(race_obs_I_recent, 2, function(x) round(x/sum(x),4))
race_obs_I_est.final <- round(I_current * race_obs_I_recent.REMOVED.frac)

race_obs_D_recent.REMOVED.frac <- apply(as.data.frame(race_observed_D_recent.nominal.REMOVED), 2, function(x) round(x/sum(x),4))
race_obs_D_est.final <- round(D_current * race_obs_D_recent.REMOVED.frac)

### CFR observed with estimated SNF removed
CFR_obs_with_SNF_removed <- round(race_obs_D_est.final / race_obs_I_est.final , 4)
...
# 
#   traj <- dplyr::mutate(TEST.out, CFRobs=(D/Idetectcum), CFRactual=(D/(Itotcum)), CFRobs.white <-  )
