...
## Probability of severe disease by race

This table shows the model estimated probability of severe disease by race/ethnicity group based on the prevalence of the risk factors in each race/ethnicity group in L.A. County. Although the nominal values of the probabilities do not differ significantly across races (for example, the probabilities for $Pr(Death|Illness)$ vary from a minimum of 0.05 to a maximum of 0.14), the relative differences between the race/ethnicity groups can translate to meaningful differences at the level of the population. To visualize these relative differences, we have depicted these probabilities using color bars sized to a normalized scale for which the color bar for the largest probability value in a column spans the column width and the lowest probability for that column is shown with a very thin bar of color.

With the normalized scaling, this chart can be used to compare the relative risk of severe illness from COVID-19 across the race/ethnicity groups. Some insights this provides:
...
It is important to recall that our model for risk of severe illness accounts only for biological risk factors for severe illness. There are many other factors that may affect the spread of the disease, the severity of illness, and the numbers appearing in healthcares statistics. Differences observed in the distribution of deaths across these groups from those projected by our model indicates these factors beyond the biological susceptibility to COVID-19 disease severity are at play. More details are provided in [Methods](https://uscbiostats.github.io/COVID19/method.html#conclusions).
...
formattable(risk.probs.table,
            list(
              ...
              `Pr(ICU|Hosptial)` = color_bar('violet', fun = unit.scale),
                `Pr(Death|ICU)` = color_bar('pink', fun = unit.scale),
                `Pr(Death|Illness)` = color_bar('red', fun = unit.scale)
            ))
