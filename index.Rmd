...
week_par_mean = as.data.frame(matrix(0, ncol = 7, nrow = n_rec))
week_par_sd = as.data.frame(matrix(0, ncol = 7, nrow = n_rec))
...
## SUMMARY STATISTICS COMPUTED ON DATA
summarydata <- sum.stats.SIMTEST(latest_data, no_obs=52) #,last_date_data = "2021-01-18")
model.R <- model.1sim.stats.no.R  # From supporting_functi
...
week_par_mean[2,] = colMeans(ABC_out$param)
week_par_sd[2,] = colStdevs(ABC_out$param)
...
```{r}
## Note: Beta(t), Alpha(t), Kappa(t), Delta(t) are read in from file
prior_param_input_week3 <- path(code.paper.dir, "prior_param_w3.R")
source(prior_param_input_week3)

## SUMMARY STATISTICS COMPUTED ON DATA
summarydata <- sum.stats.SIMTEST(latest_data,last_date_data = "2021-01-18")
model.R <- model.1sim.stats.no.R  # From supporting_function


## LOAD DATA PROCESSING AND MODEL ESTIMATING FUNCTIONS
## for week 1 later
model_functions_week3<- path(code.paper.dir, "data_model_fun_w2.R")
source(model_functions_week3)
...
ABC.out.mat <- ABC_out$param
ABC.par.out_week_3 <- as.data.frame(ABC_out$param)


week_par_mean[3,] = colMeans(ABC_out$param)
week_par_sd[3,] = colStdevs(ABC_out$param)
week_par_all[[3]] <- ABC.par.out_week_3
```
